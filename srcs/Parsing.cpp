...
/*   Updated: 2023/08/08 16:02:24 by jmorneau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool Parsing::checkData(std::string dataUrl){
  ...
  try {
    ...
    while (std::getline(archivo, line)){
      ...
      // check "{}"
      f_check_close(line, &find_server, &find_locacation);
...
void Parsing::f_check_close(std::string line, bool *find_server, bool *find_locacation){
...
void    Parsing::f_save_default_location(){
    ...
    for (size_t p = 0; p < server_check.get_ports_size(); p++){
      ...
      for (size_t ii = i + 1; ii < this->get_server_size(); ii++){
        ...
        for (size_t pp = 0; pp < server_compare.get_ports_size(); pp++){
          if (port_check == server_compare.get_ports(p))
            throw formatWrong();
      }
      }
    }
